...
if uploaded_file is not None:
    ...
    if ret:
        ...
        with col_canvas:
            ...
            # --- Visual Overlap Strategy (Corrected) ---
            # 1. Render the Image as a standard HTML element.
            st.markdown(
                f'<div style="width:{display_w}px; height:{display_h}px; overflow:hidden;">'
                f'<img src="{img_data_url}" style="width:100%; height:100%; object-fit:contain;">'
                f'</div>',
                unsafe_allow_html=True
            )
            
            # 2. Inject CSS to move the *next* iframe (the canvas) UP to overlap the image.
            # We use a unique ID based on time to ensure we target this specific block if possible, 
            # but simpler is to target the iframe generally within this container flow.
            st.markdown(
                f"""
                <style>
                /* Target the iframe generated by st_canvas */
                iframe[title="streamlit_drawable_canvas.st_canvas_wrapper"] {{
                    margin-top: -{display_h}px !important;
                    position: relative;
                    z-index: 10;
                }}
                </style>
